async function createSidebar() {
  ...
  document.getElementById("define-button").addEventListener("click", () => {
    ...
    if (word.trim()) {
      lookupInCSV('OPTED-Dictionary.csv', word.toString()).then(result => {
        document.getElementById(
          "definition-display"
        ).textContent = result.slice(3, result.length-4);
      });
    }
  });
...
// function to lookup definitions in csv file
async function lookupInCSV(csvFilePath, lookupValue, keyColumnIndex = 0, returnColumnIndex = 3) {
  try {
    const url = chrome.runtime.getURL(csvFilePath);
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (values[keyColumnIndex].toLowerCase() == lookupValue.toLowerCase()) {
        return values[returnColumnIndex];
      }
    }
    return "\"\"\"No definition found\"\"\""; // definition not found
  } catch (error) {
